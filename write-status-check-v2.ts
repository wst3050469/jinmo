...
const documentContent = readFileSync('/workspace/projects/部署AI状态报告-第二次检查.md', 'utf-8');
...
writeToKnowledgeBase().then(() => {
  ...
  console.log('========================================');
  console.log('✅ 知识库已读取');
  console.log('⏳ 无pending部署任务（路径：/workspace/projects/handover/）');
  console.log('⏳ 等待开发AI在正确路径创建部署包');
  console.log('========================================');
}).catch((error) => {
  console.error('\n❌ 写入失败:', error);
  process.exit(1);
